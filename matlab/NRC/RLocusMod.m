function RLocusMod(numG,denG,numD,denD,K,axes)
% function RLocus(numG,denG,numD,denD,K,axes)
% Plot root locus of K*D(s)*G(s) w.r.t. the extra gain K, where D(s)=numD(s)/denD(s) and
% G(s)=numG(s)/denG(s).  Convenient plotting parameters:
% K is the gains used, and axes is the axis limits.  The roots for K=1 are marked (*).
...
for j=1:length(K); denH=PolyAdd(K(j)*numL,denL);
...
axis equal, a=axes; axis(a), plot([a(1) a(2)],[0 0],'k-'), plot([0 0],[a(3) a(4)],'k-')
end % function RLocus
